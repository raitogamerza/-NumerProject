...
function GaussSeidel() {
  ...
  const [cgRecords, setCgRecords] = React.useState([]);
  const decimals = (() => {
  ...
  const handleCalculate = () => {
    try{
      const { detA, X, singular, iterations } = conjugateGradientMethod(A, B, X0, eps);
      if (singular) {
      ...
      setDetA(detA);
      setX(X);
      setCgRecords(Array.isArray(iterations) ? iterations : []);
    } catch (e) {
      alert(e.message);
    }
  }
  ...
  return (
    <div className='w-full flex justify-center'>
      <div className='w-full max-w-7xl p-4'>
        <h1 className = "text-2xl font-bold mb-4 text-center">Conjugate Gradient</h1>
        ...
        <h2 className='meta-4 text-lg font-semibold mb-2'>Result:</h2>
            ...
            {/* Iterations Table */}
            {Array.isArray(cgRecords) && cgRecords.length > 0 && (
          <div className='mt-6 w-11/12 md:w-5/6'>
            ...
            <div className='max-h-72 overflow-y-auto border border-gray-300 rounded shadow-sm'>
              <table className='min-w-full border-collapse text-sm'>
                ...
                <tbody>
                  {cgRecords.map((vec, idx) => {
                    const prev = idx > 0 ? cgRecords[idx - 1] : null;
                    let err = '-';
